...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		if ( $this->is_available( array() ) ) {
			...
			add_action( 'woocommerce_after_template_part',     array( $this, 'review_order_shipping_pickup_location' ), 10, 4 ); // WC 2.1+
	...
	/**
	 * Calculate shipping.  This is called only when the shipping package
	 * (which includes the order items) is not found in the transient
	 * storage.
	 *
	 * "Overridden" from WC_Shipping_Method, although that class doesn't
	 * actually define this method, it's assumed to exist by the client
	 * code
	 *
	 * @param array $shipping order data used to calulcate shipping, including order items
	 */
	public function calculate_shipping( $package ) {
		...
		// if there's a discount for local pickup, add a note to the displayed shipping method title
		if ( $this->get_discount() ) {
			$discount_display = is_float( $this->get_discount() ) ? wc_price( $this->get_discount() ) : $this->get_discount();
			/* translators: %s - discount amount when using this shipping method */
			$label .= ' (' . sprintf( __( 'save %s', 'woocommerce-shipping-local-pickup-plus' ), strip_tags( $discount_display ) ) . ')';
		}
	 ...
	 * * enabled
	 * * 1 or more pickup locations defined
	 * * If categories pickup only is enabled, there must be at least one product from the category in the cart
	 *
	 * Overridden from parent class
	 *
	 * @param array $package array of order data, including order items
	 *
	 * @return boolean whether this shipping method is available
	 */
	public function is_available( $package ) {

		if ( ! $this->is_enabled() || 0 == count( $this->pickup_locations ) ) return false;

		if ( 'yes' == $this->categories_pickup_only && ! empty( $this->categories ) && ! empty( $package ) ) {
	...
	public function admin_options() {
		...
		?>
		<style type="text/css">
			.shippingrows tr td:first-child input { margin: 0 0 0 8px; }
			.shippingrows tr th { white-space:nowrap; }
			<?php if ( version_compare( $wp_version, '3.8', '>=' ) ) : ?>
				.shippingrows.widefat tr .check-column { padding-top:20px; }
				.shippingrows tfoot tr th { padding-left:12px; }
			<?php endif; ?>
		</style>
	...
	/**
	 * Helper function to run the given variable through the woocommerce_clean()
	 * and stripslashes_deep() functions
	 *
	 * @since 1.6
	...
	public function process_admin_options() {
		...
		for ( $i = 0, $ix = count( $ids ); $i < $ix; $i++ ) {
			...
			$pickup_location = array(
				...
				'cost'      => isset( $costs[ $i ] )     ? abs( $costs[ $i ] )    : null,
				'id'        => $id,
	...
	/**
	 * If this is our special Pickup coupon, remove/change the coupon message
	 *
	 * This change {@link https://github.com/woothemes/woocommerce/pull/7648}
	 * allows us to remove the coupon message and wc notice entirely. For older
	 * WC versions, we adjust the message to something more appropriate
	 *
	 *
	 * @since 1.10.1
	 * @param string $msg the coupon message
	 * @param int $msg_code one of 200 or 201
	 * @param WC_Coupon $coupon the coupon
	 */
	public function coupon_remove_message( $msg, $msg_code, $coupon ) {
		...
		if ( $coupon->code == $this->get_discount_code() ) {
			...
			// TODO: once https://github.com/woothemes/woocommerce/pull/7648 is accepted
			// if ( SV_WC_Plugin_Compatibility::is_wc_version_gt( 'xxx' ) ) {
			if ( false ) {
				$msg = false;
			} else {
				switch ( $msg_code ) {
					case WC_Coupon::WC_COUPON_SUCCESS :
						$msg = __( 'Local pickup discount added.', 'woocommerce-shipping-local-pickup-plus' );
					break;

					case WC_Coupon::WC_COUPON_REMOVED :
						$msg = __( 'Local pickup discount removed.', 'woocommerce-shipping-local-pickup-plus' );
					break;
				}
			}
		}
	...
	public function get_discount_data( $data, $code ) {
		...
		} else if ( preg_match( '/(\d+\.?\d*)%/', $discount, $matches ) ) {
			$data = array(
				'discount_type' => 'percent',
				'coupon_amount' => $matches[1],
			);
		}
	...
	public function get_discount_code() {
		if ( ! is_null( WC()->session ) ) {
			return WC()->session->get( 'wc_local_pickup_plus_discount_code' );
		}
	}
	 ...
	 *
	 * @since 1.4.4
	 * @return float|string the discount amount, as absolute float or as a string percentage
	 */
	private function get_discount() {

		$discount_amount = 0;

		// no coupons, no support for discounts
		if ( ! $this->coupons_enabled() ) {
			return $discount_amount;
		}

		if ( ( is_numeric( $this->discount ) && (float) $this->discount > 0 ) ) {
			$discount_amount = (float) $this->discount;
		} elseif ( preg_match( '/(\d+\.?\d*)%/', $this->discount, $matches ) ) {
			$discount_amount = $this->discount;
		}

		return apply_filters( 'wc_shipping_local_pickup_plus_discount_amount', $discount_amount );
	}
	...
	public function update_shipping_packages( $packages ) {
		...
		foreach ( $packages as $package_index => $package ) {
			...
			if ( count( $other_products ) == 0 && count( $found_products ) > 0 ) {
				...
				// create a package containing only the local pickup plus category products
		...
		foreach ( $new_packages as $package_index => $package ) {
			if ( isset( $chosen_shipping_methods[ $package_index ] ) && $this->id == $chosen_shipping_methods[ $package_index ] && $this->has_chosen_pickup_location( $package_index ) ) {
				$new_packages[ $package_index ]['pickup_location'] = $this->get_chosen_pickup_location_id( $package_index );
			}
		}
	...
	public function review_order_shipping_pickup_location( $template_name, $template_path, $located, $args ) {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_4() ) {
			...
			$is_ajax = defined( 'DOING_AJAX' ) && isset( $_REQUEST['action'] ) && 'woocommerce_update_order_review' == $_REQUEST['action'];
		}

		// after the cart shipping template partial, for a shipping package with local pickup plus as an option
		//  while on the checkout page (or update checkout ajax request).  This info cannot be edited on the pay
		//  page
		// Note: technically we could now allow the pickup location to be selected on the cart, by removing the
		//  'is_checkout()' check here, however that will require yet more handling code...
		if ( 'cart/cart-shipping.php' == $template_name && $this->id == $args['chosen_method'] && ( is_checkout() || $is_ajax ) ) {
			...
			do_action( 'woocommerce_review_order_before_local_pickup_location', $this->pickup_locations, $args );
			...
			if ( 1 == count( $this->pickup_locations ) ) {
				...
				echo '<input type="hidden" name="pickup_location[' . $args['index'] . ']" value="' . esc_attr( $chosen_pickup_location['id'] ) . '" />';
			} else {

				$chosen_pickup_location_id = $this->get_chosen_pickup_location_id( $args['index'] );
				$chosen_pickup_location    = null;

				if ( 'select' == $this->get_checkout_pickup_location_styling() ) {
					echo '<select name="pickup_location[' . $args['index'] . ']" class="pickup_location" data-placeholder="' . __( 'Choose One', 'woocommerce-shipping-local-pickup-plus' ) . '" style="width: 100%;">';
					...
					foreach ( $this->pickup_locations as $location ) {
						...
						echo '<input type="radio" name="pickup_location[' . $args['index'] . ']" id="pickup_location_' . $args['index'] . '_' . $location['id'] . '" value="' . esc_attr( $location['id'] ) . '" class="pickup_location" ' . checked( $location['id'], $chosen_pickup_location_id, false ) . ' />';
						echo '<label for="pickup_location_' . $args['index'] . '_' . $location['id'] . '">' . $this->get_formatted_address_helper( $location, true, true ) . '</label>';
						echo '</li>';
					}
					echo '</ul>';
				}
			...
			if ( $chosen_pickup_location && isset( $chosen_pickup_location['note'] ) && $chosen_pickup_location['note'] ) {
				echo '<div class="wc-pickup-location-note">' . $chosen_pickup_location['note'] . '</div>';
			}
	 ...
	 * pre WC 2.1
	 *
	 * @deprecated no longer used since WooCommerce version 2.1 (replaced by review_order_shipping_pickup_location())
	 */
	public function review_order_pickup_location() {

		if ( in_array( $this->id, $this->get_chosen_shipping_methods() ) ) {
			...
			echo '<tr class="pickup_location">';
			echo '<th colspan="1">' . ( 1 == count( $this->pickup_locations ) ? __( 'Your Pickup Location', 'woocommerce-shipping-local-pickup-plus'  ) : __( 'Choose Pickup Location', 'woocommerce-shipping-local-pickup-plus' ) ) . '</th>';
			echo '<td class="update_totals_on_change">';

			do_action( 'woocommerce_review_order_before_local_pickup_location', $this->pickup_locations );

			if ( 1 == count( $this->pickup_locations ) ) {

				$location = $this->pickup_locations[0];
				echo $this->get_formatted_address_helper( $location, true );

				echo '<input type="hidden" name="pickup_location[0]" value="' . esc_attr( $location['id'] ) . '" />';
			} else {
				echo '<select name="pickup_location[0]" class="pickup_location wc-enhanced-select" data-placeholder="' . __( 'Choose One', 'woocommerce-shipping-local-pickup-plus' ) . '">';
				echo '<option value=""></option>';
				foreach ( $this->pickup_locations as $location ) {
					echo '<option value="' . esc_attr( $location['id'] ) . '" ' . selected( $location['id'], $this->get_chosen_pickup_location_id( 0 ), false ) . '>';
					echo $this->get_formatted_address_helper( $location, true, true );
					echo '</option>';
				}
				echo '</select>';
	...
	public function checkout_update_order_review( $post_data ) {
		...
		foreach ( $post_data as $data ) {
			...
			if ( false !== strpos( $data, '=' ) ) {
				...
				if ( preg_match( '/pickup_location\[(\d+)\]/', $name, $matches ) ) {
	...
	/**
	 * Set the selected pickup location on the order record when it's created,
	 * if the shipping method is local pickup plus
	 *
	 * WooCommerce < 2.1
	 *
	 * @param int $order_id order identifier
	...
	public function checkout_update_order_meta( $order_id, $posted ) {
		...
		if ( $this->id == $shipping_method && isset( $_POST['pickup_location'][0] ) ) {
			$location = $this->get_pickup_location_by_id( $_POST['pickup_location'][0] );
			if ( $location ) {
				update_post_meta( $order_id, '_pickup_location', $location );
				WC()->session->set( 'chosen_pickup_locations', array( $location['id'] ) );
			}
		}
	}
	 ...
	 *
	 * @since 1.10.1
	 * @return boolean true if this gateway is enabled
	 */
	...
	/**
	 * Returns the configured checkout pickup location styling, one of 'dropdown' or 'radio'
	 *
	 * @since 1.7
	 * @return string one of 'dropdown' or 'radio'
	 */
	public function get_checkout_pickup_location_styling() {

		if ( $this->checkout_pickup_location_styling ) {
			return $this->checkout_pickup_location_styling;
		}
	 ...
	 *
	 * @return array with the minimum cost as the first element, and the
	 *         maximum cost as the second element
	 */
	...
	/**
	 * Load the configured pickup locations from the database and set them to
	 * the pickup_locations member
	 *
	 * @return void
	 */
	 ...
	 *
	 * @return array associative array of address fields available for
	 *         $country
	 */
	 ...
	 *
	 * @return mixed numeric cost if non-zero, or empty string otherwise
	 */
	 ...
	 * @param boolean $one_line whether to return the the address on
	 *        one line, without the phone number, of broken up onto
	 *        multiple lines, ending with the phone number if present
	 * @param boolean $show_cost whether to display the cost, if any
