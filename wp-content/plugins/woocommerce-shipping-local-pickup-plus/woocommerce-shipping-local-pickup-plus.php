...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.2.0', __( 'WooCommerce Local Pickup Plus', 'woocommerce-shipping-local-pickup-plus' ), __FILE__, 'init_woocommerce_shipping_local_pickup_plus', array( 'minimum_wc_version' => '2.3.6', 'backwards_compatible' => '4.2.0' ) );
...
class WC_Local_Pickup_Plus extends SV_WC_Plugin {
	...
	/** plugin text domain, DEPRECATED as of 1.12.0 */
	...
	public function __construct() {
		...
		parent::__construct( self::METHOD_ID, self::VERSION );
	...
	public function get_discount_data( $data, $code ) {
		...
		if ( in_array( self::METHOD_ID, $chosen_shipping_methods ) ) {
			$local_pickup_plus = $this->get_local_pickup_plus();
			$data = $local_pickup_plus->get_discount_data( $data, $code );
		}
	...
	public function frontend_scripts() {
		...
		if ( is_checkout() ) {
			...
			if ( $local_pickup_plus->is_available( array() ) && $local_pickup_plus->hide_shipping_address() ) {
				ob_start();
	 ...
	 * 'customer note' admin new order' emails for WC 2.3+
	 *
	 * @since 1.11.0
	 */
