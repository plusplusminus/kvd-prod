...
class WC_Wishlists_Email_Verification {
	...
	private function __construct() {
		add_action( 'init', array($this, 'on_init'), 8 );
	...
	public function on_wishlist_created( $wishlist_id, $args ) {
		...
		if ( is_user_logged_in() && !empty( $args['wishlist_owner_email'] ) && $current_user->user_email == $args['wishlist_owner_email'] ) {
			...
			if ( !empty( $args['wishlist_owner_email'] ) ) {
				...
				$message = 'We received your request to add your email to your wishlist.  Before we begin using this email address, we want to be certian we have your permission. Confirm by visiting this link in your browser <a href="' . $link . '">' . $link . '</a>.  If you did not make this request you can safely ignore this email.';
		...
		if (!empty( $args['wishlist_owner_email'] ) && ( 
			...
			if ( !$is_valid ) {
				...
				if ( !empty( $args['wishlist_owner_email'] ) ) {
					...
					$link = add_query_arg( array('wcwlemailconfirmationcode' =>  $random_hash), get_site_url() );
					$message = 'We received your request to add your email to your wishlist.  Before we begin using this email address, we want to be certian we have your permission. Confirm by visiting this link in your browser ' . $link . ' .  If you did not make this request you can safely ignore this email.';
