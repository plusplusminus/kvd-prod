	...
	private static function add_to_cart( $wishlist_id = false, $wishlist_item_key = false, $suppress_messages = false ) {
		...
		if ( $wishlist_item_key ) {
			if ( sizeof( $wishlist_items > 0 ) && isset( $wishlist_items[$wishlist_item_key] ) ) {
				...
				$check_cart_id = $woocommerce->cart->generate_cart_id( (int) $cart_item['product_id'], $cart_item['variation_id'], $cart_item['variation'], $add_on_data );

				// See if this product and its options is already in the cart
				$check_cart_item_key = $woocommerce->cart->find_product_in_cart( $check_cart_id );

				$product_data = get_product( isset( $cart_item['variation_id'] ) && !empty( $cart_item['variation_id'] ) ? $cart_item['variation_id'] : $cart_item['product_id']  );
				if ( !$product_data ) {
				...
				if ( $product_data->product_type == 'external' ) {
				...
				if ( $product_data->is_sold_individually() ) {
					$in_cart_quantity = $check_cart_item_key ? $woocommerce->cart->cart_contents[$check_cart_item_key]['quantity'] : 0;

					if ( $in_cart_quantity > 0 ) {
						WC_Wishlist_Compatibility::wc_add_notice( sprintf(
								'<a href="%s" class="button wc-forward">%s</a> %s', $woocommerce->cart->get_cart_url(), __( 'View Cart', 'woocommerce' ), sprintf( __( 'You cannot add another &quot;%s&quot; to your cart.', 'woocommerce' ), $product_data->get_title() )
							), 'error' );
						return false;
					}
				}
