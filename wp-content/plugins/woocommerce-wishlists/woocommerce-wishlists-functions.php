...
function woocommerce_wishlists_get_template($template_name, $args = array(), $template_path = '') {
	global $woocommerce;
	$args = array_merge($args, array('woocommerce'=>$woocommerce) );
...
function woocommerce_wishlists_is_product_in_wishlist($product_id, $wishlist_id = false) {
	if ($wishlist) {
		...
		return isset($product_ids[$product_id]) && in_array($wishlist, $product_ids[$product_id]);
	}
...
function woocommerce_wishlist_url_item_remove($wishlist_id, $wishlist_item_key) {
	...
	return WC_Wishlists_Plugin::nonce_url('wishlists-remove-from-list', add_query_arg(array('wlaction' => 'wishlists-remove-from-list', 'wlid' => $wishlist_id, 'wishlist-item-key' => $wishlist_item_key), WC_Wishlists_Pages::get_url_for('edit-my-list')));
}
...
function woocommerce_wishlist_url_item_add_to_cart($wishlist_id, $wishlist_item_key, $wishlist_sharing_key = false, $wishlist_mode = false) {
	...
	if ($wishlist_mode == 'edit') {
		return WC_Wishlists_Plugin::nonce_url('add-cart-item', add_query_arg(array('wlkey' => $wishlist_sharing_key, 'wlaction' => 'add-cart-item', 'wlid' => $wishlist_id, 'wishlist-item-key' => $wishlist_item_key), WC_Wishlists_Wishlist::get_the_url_edit($wishlist_id)));
	} else {
		return WC_Wishlists_Plugin::nonce_url('add-cart-item', add_query_arg(array('wlkey' => $wishlist_sharing_key, 'wlaction' => 'add-cart-item', 'wlid' => $wishlist_id, 'wishlist-item-key' => $wishlist_item_key), WC_Wishlists_Wishlist::get_the_url_view($wishlist_id)));
	}
}
...
function woocommerce_wishlist_url_add_all_to_cart($wishlist_id, $wishlist_sharing_key = false, $wishlist_mode = false) {
	...
	if ($wishlist_mode == 'edit') {
		$url = WC_Wishlists_Plugin::nonce_url('add-cart-items', add_query_arg(array('wlaction' => 'add-cart-items', 'wlkey' => $wishlist_sharing_key, 'wlid' => $wishlist_id), WC_Wishlists_Wishlist::get_the_url_edit($wishlist_id)));
	} else {
		$url = WC_Wishlists_Plugin::nonce_url('add-cart-items', add_query_arg(array('wlaction' => 'add-cart-items', 'wlkey' => $wishlist_sharing_key, 'wlid' => $wishlist_id), WC_Wishlists_Wishlist::get_the_url_view($wishlist_id)));
	}
...
function _woocommerce_wishlist_sort_item_collection_price_asc($a, $b) {
	...
	$prod_a_price = get_option('woocommerce_display_cart_prices_excluding_tax') == 'yes' || $woocommerce->customer->is_vat_exempt() ? $prod_a->get_price_excluding_tax() : $prod_a->get_price();
	$prod_b_price = get_option('woocommerce_display_cart_prices_excluding_tax') == 'yes' || $woocommerce->customer->is_vat_exempt() ? $prod_b->get_price_excluding_tax() : $prod_b->get_price();
...
function _woocommerce_wishlist_filter_item_collection_category($item_collection, $category) {
	...
	foreach ($item_collection as $key => $item) {
		$prod = $item['data'];
		if (is_object_in_term($prod->id, 'product_cat', $category)) {
	...
	function woocommerce_wishlist_render_email_forms() {
		...
		if ($email_forms && !empty($email_forms)) :
			...
			foreach ($email_forms as $wishlist) {
				...
				<div class="wl-modal hide" id="share-via-email-<?php echo $wishlist->id; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display:none;z-index:9999;">
